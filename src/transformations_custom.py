"""
Custom Signal Transformations for Functional Data Analysis

This module contains the ORIGINAL custom implementations of B-spline and FPCA
transformations that were used before switching to scikit-fda. These implementations
achieved significantly better results (JH R² ≈ 0.61) compared to scikit-fda (JH R² ≈ 0.34).

Key differences from scikit-fda:
- Uses DISCRETE dot products instead of L² inner products
- Covariance computed as X.T @ X (discrete) vs numerical integration (L²)
- Projection computed as X @ eigenfunctions (discrete) vs L² projection
- Simple pointwise mean function vs L² mean

This file is provided for comparison and potential recovery of original performance.

Usage:
    from transformations_custom import get_custom_transformer

    transformer = get_custom_transformer(
        transform_type='fpc',
        n_components=15,
        use_varimax=False,  # Recommended
    )
"""
...
class CustomBSplineTransformer(BaseSignalTransformer):
    """
    B-spline basis representation with smoothing (ORIGINAL CUSTOM IMPLEMENTATION).

    Represents signals as linear combinations of B-spline basis functions,
    with roughness penalty for smoothing.

    Key features:
    - Manually constructs basis matrix using scipy.interpolate.BSpline
    - Builds roughness penalty matrix via second derivative integral
    - Solves least squares: c = (B'B + lambda*P)^-1 @ B' @ y
    ...
    def fit(self, signals: np.ndarray) -> 'CustomBSplineTransformer':
        """
...
class CustomFPCATransformer(BaseSignalTransformer):
    """
    Functional PCA transformer using DISCRETE dot products (ORIGINAL CUSTOM IMPLEMENTATION).

    This implementation achieved JH R² ≈ 0.61 before being replaced by scikit-fda
    (which achieves JH R² ≈ 0.34). The key difference is:

    - This uses DISCRETE dot products: cov = X.T @ X, scores = X @ eigenfunctions
    ...
    weight by the integration rule.
    ...
    Args:
        ...
        use_varimax: Whether to apply varimax rotation to redistribute variance
            NOTE: Recommended to set False based on experiments
    ...
    def __init__(
        ...
        use_varimax: bool = False,  # Changed default to False based on experiments
    ...
    def fit(self, signals: np.ndarray) -> 'CustomFPCATransformer':
        """
        Fit FPCA to training signals using DISCRETE dot products.

        Key difference from scikit-fda:
        - Covariance: X.T @ X / (n_samples - 1) [discrete sum]
        - NOT: integral of X(t) * X(s) dt ds [L² inner product]
        ...
        # Compute mean function per channel (POINTWISE mean, not L² mean)
        ...
        for ch in range(n_channels):
            ...
            # Compute covariance matrix using DISCRETE dot product: (seq_len, seq_len)
            # This is the key difference from scikit-fda which uses L² inner products
    ...
    def transform(self, signals: np.ndarray) -> np.ndarray:
        """
        Transform signals to FPC scores using DISCRETE dot product.

        Key difference from scikit-fda:
        - scores = X @ eigenfunctions [discrete sum]
        - NOT: integral of X(t) * eigenfunction(t) dt [L² inner product]
        ...
        for ch in range(n_channels):
            ...
            # Project onto eigenfunctions using DISCRETE dot product
            # scores = X @ eigenfunctions
    ...
    def get_eigenvalues(self) -> np.ndarray:
        """
        Get eigenvalues (variance explained) for each component.

        Returns:
            Array of shape (n_components, n_channels) with eigenvalues.
        """
    ...
    def get_inverse_transform_components(self) -> dict:
        ...
        mean_functions = []
        for ch in range(self._n_channels):
            mean_functions.append(self._mean_function[:, ch].astype(np.float32))

        return {
            'eigenfunctions': [ef.astype(np.float32) for ef in self._eigenfunctions],
            'mean_functions': mean_functions,
            'rotation_matrices': self._rotation_matrices,
            'score_std': None,  # Not used in custom implementation
...
def get_custom_transformer(
    ...
    use_varimax: bool = False,  # Default to False based on experiments
) -> BaseSignalTransformer:
    """
    Factory function to create CUSTOM signal transformers.

    These are the original implementations that achieved better results
    than scikit-fda (JH R² ≈ 0.61 vs 0.34).
    ...
    Args:
        ...
        use_varimax: Whether to apply varimax rotation to FPCs (default: False)
    ...
    if transform_type == 'raw':
        ...
        return CustomBSplineTransformer(
            n_basis=n_basis,
            smoothing_lambda=bspline_lambda,
        )
    elif transform_type == 'fpc':
        return CustomFPCATransformer(
...
def compare_transformers(signals: np.ndarray, n_components: int = 15) -> dict:
    """
    Compare custom vs scikit-fda FPCA implementations.
    ...
    """
    # Import scikit-fda transformer for comparison
    from src.transformations import FPCATransformer as SkfdaFPCATransformer

    # Fit both transformers
    custom = CustomFPCATransformer(n_components=n_components, use_varimax=False)
    skfda = SkfdaFPCATransformer(n_components=n_components, use_varimax=False)

    custom.fit(signals)
    skfda.fit(signals)

    # Get scores
    custom_scores = custom.transform(signals)
    skfda_scores = skfda.transform(signals)
    ...
    # Compute errors
    custom_error = signals - custom_recon
    skfda_error = signals - skfda_recon

    return {
        'custom': {
            'rmse': np.sqrt(np.mean(custom_error ** 2)),
            'score_range': (custom_scores.min(), custom_scores.max()),
            'score_std': np.std(custom_scores),
            'variance_explained': custom.get_variance_explained(),
        },
        'skfda': {
            'rmse': np.sqrt(np.mean(skfda_error ** 2)),
            'score_range': (skfda_scores.min(), skfda_scores.max()),
            'score_std': np.std(skfda_scores),
            'variance_explained': skfda.get_variance_explained(),
        },
        'score_correlation': np.corrcoef(
            custom_scores[:, 0, 0], skfda_scores[:, 0, 0]
        )[0, 1] if custom_scores.shape[1] > 0 else np.nan,
    }
