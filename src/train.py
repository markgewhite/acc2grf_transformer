...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=str,
        default='mse',
        choices=['mse', 'jump_height', 'peak_power', 'combined', 'weighted', 'smooth', 'eigenvalue_weighted', 'reconstruction', 'reconstruction_weighted'],
        help='Loss function (default: mse). reconstruction_weighted applies jerk-based temporal weighting.'
    )
    parser.add_argument(
        '--mse-weight',
        type=float,
...
def run_single_trial(args, trial_seed: int, paths: dict, train_ds, val_ds, info, use_resultant: bool, use_varimax: bool, scalar_prediction, loader) -> dict:
    ...
    # Get loss function
    temporal_weights = info.get('temporal_weights') if args.loss in ['weighted', 'reconstruction_weighted'] else None
    ...
    # Note: inverse_transform_components was used by removed signal_space loss
    inverse_transform_components = None

    # Get reconstruction components for reconstruction/reconstruction_weighted loss (requires bspline or fpc)
    reconstruction_components = None
    if args.loss in ['reconstruction', 'reconstruction_weighted']:
        if args.output_transform == 'raw':
            raise ValueError(f"{args.loss} loss requires --output-transform bspline or fpc")
        output_transformer = info.get('output_transformer')
        if output_transformer is None or not hasattr(output_transformer, 'get_reconstruction_components'):
            raise ValueError(f"{args.loss} loss requires a transformer with get_reconstruction_components()")
    ...
    if args.loss == 'reconstruction':
        print(f"  Computing loss in signal space using reconstruction matrix (unweighted)")
    if args.loss == 'reconstruction_weighted':
