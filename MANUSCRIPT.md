...
Before further analysis, both accelerometer and GRF signals were converted from discrete samples to smooth functional representations. Each signal was expressed as a linear combination of 50 cubic B-spline basis functions with a roughness penalty on the second derivative to suppress high-frequency noise while preserving the biomechanically relevant signal features (Ramsay & Silverman, 2005). The roughness penalty parameter λ was selected independently for each signal channel using generalised cross-validation (GCV), which balances fidelity to the observed data against smoothness of the fitted function. The GCV-optimal λ was fitted on the training set and applied to both training and validation data. The smoothed functional representations were then re-evaluated at the original 500 time points, yielding noise-free discrete signals that served as the common starting point for all three representation strategies described below.
...
**B-spline coefficients.** Each signal was represented as a weighted sum of 30 cubic B-spline basis functions (de Boor, 2001). The B-spline coefficients provide a reduced-dimensional representation (30 coefficients per channel), but the coefficients have no variance ordering and no inherent biomechanical structure. The input comprised 30 coefficients × 3 channels = 90 values; the output comprised 30 coefficients × 1 channel = 30 values.

**FPC scores.** Each signal was decomposed via FPCA into a mean function plus variance-ordered deviations, as described in Section 2.3. The input comprised 15 FPC scores × 3 channels = 45 values; the output comprised 15 FPC scores × 1 channel = 15 values. The mean functions encode the canonical CMJ shapes and are fixed; the model maps only the score vectors. This representation achieves the greatest dimensionality reduction (45 → 15) and provides a variance-ordered, biomechanically structured input space.

For each representation, the same training protocol, 5-trial validation procedure, and evaluation metrics were applied. It is worth noting that for the FPC representation, the score-to-score mapping can be characterised as a linear projection — a weighted combination of input scores producing each output score. This property is exploited in Section 2.7 to provide an interpretability analysis of the learned mapping.

### 2.5 Prediction models

I compared two model architectures representing opposite ends of the complexity spectrum:

**Transformer.** An encoder-only transformer (Vaswani et al., 2017) with approximately 750,000 parameters, comprising a 64-dimensional model, 4 self-attention heads, 3 encoder layers, a 128-dimensional feed-forward network, and learnable positional encoding. The self-attention mechanism learns temporal relationships across the full input sequence, allowing each time step to attend to every other. I applied this architecture to raw signals, representing the state-of-the-art deep learning approach to sequence-to-sequence prediction. The transformer embodies the trend in the field toward increasingly complex architectures (Choi et al., 2024; Bogaert et al., 2024).
...
For each representation, the model received a different view of the same underlying smoothed signal. In the smoothed signal representation, the model operated on the re-evaluated time series directly. In the B-spline and FPC representations, the model's output was a vector of coefficients or scores rather than a time series. The full GRF waveform was reconstructed by applying the corresponding inverse transform: for B-splines, the predicted coefficients were multiplied by the basis functions; for FPC, the predicted scores were multiplied by the eigenfunctions and added to the mean function. The reconstructed waveform was then used to derive biomechanical metrics (Section 2.8).
